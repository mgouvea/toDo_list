...
export function TaskField({
  ...
  const handleClickCheck = (idx: any) => {
    setFinishedTask(!finishedTask);

    if (onFinished && !finishedTask) {
      const taskToFinished = idx;
      onFinished(taskToFinished);
    }

    if (onOpenTask && !!finishedTask) {
      onOpenTask();
    }
  };
    ...
    <div className="field">
      <div>
        <div className="fieldContent">
          <span
            className={!!finishedTask ? 'check' : 'unCheck'}
          ...
          <p className={!!finishedTask ? 'taskFinished' : 'taskUnfinished'}>
            {task}
          </p>
        </div>
      </div>
      ...
      {/* icone lixeira */}
      <div className="trashIcon" onClick={() => handleDeleteTask(index)}>
        <svg
          width="28"
          height="28"
          viewBox="0 0 24 24"
