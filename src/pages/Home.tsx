...
export function Home() {
  ...
  const [taskArrayFinished, setTaskArrayFinished] = useState([]);
  ...
  const handleDeleteTask = (t: any) => {
    taskArray.splice(t, 1);
    setUpdateScreen(!updateScreen);
  };

  const handleTaskFinished = (t: any) => {
    // @ts-ignore
    taskArrayFinished.push(t);
    setUpdateScreen(!updateScreen);
  };

  const handleOpenTask = () => {
    // @ts-ignore
    taskArrayFinished.pop();
    setUpdateScreen(!updateScreen);
  };
      ...
      <div className="content">
        <div className="createTaskContent">
          <input
            ...
            maxLength={65}
          />
          <button className="buttonCreateTask" onClick={handleCreateTask}>
            <div className="buttonCreateContent">
              <p>Criar</p>
              <svg
                width="16"
                height="16"
        ...
        <div className="statusContent">
          <div className="taskStatus">
            <div className="createTaskStatus">
              Tarefas criadas <p className="number">{taskArray.length}</p>
            </div>
            <div className="doneTaskStatus">
              Concluídas <p className="number">{taskArrayFinished.length}</p>
            </div>
          </div>
          {!taskArray.length && <BlankSpace />}
          {!!taskArray.length &&
